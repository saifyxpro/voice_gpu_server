...
echo "$TTS_JSON" | python3 -c "
import sys, json
d = json.load(sys.stdin)
if 'detail' in d:
    print('ERROR:', d['detail'])
    sys.exit(1)
print({k: d[k] for k in d if k != 'audio_base64'})
print('audio_base64 length:', len(d.get('audio_base64', '')))
" || fail "TTS failed (see error above)"
...
TEXT=$(echo "$STT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('detail') or d.get('text','').strip())")
if echo "$STT" | grep -q '"detail"'; then
  fail "STT error: $TEXT"
elif [[ -n "$TEXT" ]]; then
